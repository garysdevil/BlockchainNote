...
2. 首先进入交互界面-指令操作
   ...
   6. 添加静态连接节点。两个节点想要联通必须保证网络是可用的，并且指定相同的networkid\
   ...
   7. 查看同步了的块高/查看块高同步的百分比（未同步完成时）\
      eth.syncing\
      console.log(parseInt(eth.syncing.currentBlock/eth.syncing.highestBlock\*100,10)+'%')

   8. 账户挖矿交易相关

      01. personal.newAccount()
      02. eth.accounts
      03. eth.getBalance("账户地址")
      04. 挖矿所得账户会进入矿工账户，称呼为coinbase，默认情况下coinbase是本地账户中的第一个账户 eth.coinbase
      05. 设置矿工账户 miner.setEtherbase()
      06. wei是以太币的最小单位，一个以太币=10的18次方个wei，将wei换算为以太币web3.fromWei() web3.toWei()
      07. 交易 eth.sendTransaction(from:eth.accounts[0],to:eth.accounts[1],value:1)
      08. personal.unlockAccount(钱包地址)
      09. 查看链上正在进行交易的数量 txpool.status
...
8. gas解释 https://www.jianshu.com/p/36caba297e14
